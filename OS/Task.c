...
void checkLastPermissions(const char *file, const char *quarantineDir, FILE *corruptedFile) {
    struct stat statbuf;
    ...
    if (!(statbuf.st_mode & S_IROTH) || !(statbuf.st_mode & S_IWOTH) || !(statbuf.st_mode & S_IXOTH)) {
        fprintf(corruptedFile, "%s\n", file);

        char quarantine_filepath[1024];
        snprintf(quarantine_filepath, sizeof(quarantine_filepath), "%s/%s", quarantineDir, file);

        if (rename(file, quarantine_filepath) != 0) {
            fprintf(stderr, "Error: Unable to move file %s to Quarantine directory\n", file);
        }
    }
}

void traverseDir(const char *path, const char *quarantineDir, FILE *file) {
    DIR *dir;
    struct dirent *entry;
    struct stat statbuf;
    char timestamp[20];
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if(S_ISDIR(statbuf.st_mode)) {
            traverseDir(filepath, quarantineDir, file);
        } else {
            checkLastPermissions(filepath, quarantineDir, file);
        }
    }
    closedir(dir);
}
...
int main(int argc, char *argv[]) {
    if (argc < 4 || argc > 12) {
        fprintf(stderr, "Usage: %s <directory_path1> <directory_path2> <quarantine_directory> [<directory_path3> ... <directory_path10>]\n", argv[0]);
        return EXIT_FAILURE;
    }

    FILE *corruptedFile;
    corruptedFile = fopen("corrupted_files.txt", "w");
    if (!corruptedFile) {
        fprintf(stderr, "Error: Unable to create corrupted_files.txt\n");
        return EXIT_FAILURE;
    }

    const char *quarantineDir = argv[3];
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (pid < 0) {
            ...
            traverseDir(argv[i], quarantineDir, snapshotFile);
